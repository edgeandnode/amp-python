...
class TestIdempotencyExactlyOnce:
    ...
    def test_transactional_exactly_once_postgresql(self, postgresql_test_config, small_test_data):
        ...
        loader = PostgreSQLLoader(config_dict)
        ...
        try:
            ...
            # Define block ranges for the batch
            ranges = [
                BlockRange(network='ethereum', start=100, end=200, hash='0xabc'),
            ]
            ...
            rows_loaded = loader.load_batch_transactional(
                small_test_data.to_batches()[0], test_table, connection_name, ranges, batch_hash
            )
            assert rows_loaded == small_test_data.num_rows
            ...
            rows_loaded_2 = loader.load_batch_transactional(
                small_test_data.to_batches()[0], test_table, connection_name, ranges, batch_hash
            )
            assert rows_loaded_2 == 0, 'Second load should return 0 (duplicate detected)'
            ...
            # Verify processed ranges table has the entry
            conn = loader.pool.getconn()
            try:
                cursor = conn.cursor()
                cursor.execute("""
