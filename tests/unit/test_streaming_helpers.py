...
def mock_loader():
    ...
    # Mock state store (unified checkpoint + idempotency system)
    loader.state_store = Mock()
    loader.state_enabled = True

    # Keep legacy mocks for backward compatibility with some tests
...
class TestProcessReorgEvent:
    ...
    def test_successful_reorg_processing(self, mock_loader, sample_ranges):
        ...
        # Verify method calls
        mock_loader._handle_reorg.assert_called_once_with(sample_ranges, 'test_table', 'test_conn')
        # Verify state store invalidation was called for each range
        assert mock_loader.state_store.invalidate_from_block.call_count == 2
        # Verify it was called with correct parameters for each network
        calls = mock_loader.state_store.invalidate_from_block.call_args_list
        assert calls[0][0] == ('test_conn', 'test_table', 'ethereum', 100)
        assert calls[1][0] == ('test_conn', 'test_table', 'polygon', 300)
...
# NOTE: TestSaveCheckpointIfComplete class removed
# The _save_checkpoint_if_complete() method was removed during the unified StreamState refactor.
# Checkpoint saving is now automatically handled within state_store.mark_processed() flow.
